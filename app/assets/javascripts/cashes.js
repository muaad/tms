function setEditable() {
  ...
  $('table.cash_table td.editable').on('change', function(evt, newValue) {
    ...
    else if ( elem.hasClass( "amount_in" ) ) {
    	data = { cash: { amount_in: newValue }, _method:'put', authenticity_token: token}
    }
    ...
    $.ajax({
      ...
      success: function(data, textStatus, jqXhr) {
        $('#total-cash').html(data.total_cash)
        $('#cash-balance').html(data.balance)
        console.log(data.balance)
        console.log(data.total_cash)
      }
    });
  });
}
...
function refreshCashTable(tbl, url, data) {
	$.ajax({
	    type: "POST",
	    url: url,
	    dataType: 'json',
	    ...
	    success: function(data, textStatus, jqXhr) {
	      ...
	      $('#new-expenses-inline').after('<tr>' + check + date + lpo + category + truck + entity + quantity + unit_price + amount + description + balance + '</tr>');
...
$(function() {
	...
	if(window.location.pathname === "/cashes") {
		...
		$(document).on('keypress', '#expense_description', function(e) {
		    if(e.which == 13) {
		    	...
		    	console.log(data)
		    	refreshCashTable('#example', '/expenses/inline_create', data);
		    	$('#expense_lpo').val("")
		    	$('#expense_amount').val("")
